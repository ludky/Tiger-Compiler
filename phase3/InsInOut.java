...
public class InsInOut {
    ...
    private void findUseDef(Instruction ins) {
        if(ins instanceof Binary) {
            ...
            addIfTemp(defTemp, ((BinaryImm)ins).operand4);
        ...
        } else if(ins instanceof BranchImm) { 
        	addIfTemp(useTemp, ((BranchImm)ins).operand1);
        } else if (ins instanceof Assignment) {
			addIfTemp(defTemp, ((Assignment)ins).operand1);
			addIfTemp(useTemp, ((Assignment)ins).operand2);
        } else if (ins instanceof AssignImm) {
        	AssignImm curr = (AssignImm) ins;
        	addIfTemp(defTemp, curr.operand1);
        }  else if (ins instanceof Return) {
        	Return curr = (Return) ins;
        	if (curr.operand1 != null) {
        		addIfTemp(useTemp, ((Return)ins).operand1);
        	}
            ...
            Callr curr = (Callr) ins;
            for (Temp t : curr.param) {
            	addIfTemp(useTemp, t);
            }
            addIfTemp(defTemp, curr.operand);
        } else if (ins instanceof ArrayStore) {
            ArrayStore curr = (ArrayStore) ins;
            addIfTemp(defTemp, curr.arrayname);
            addIfTemp(useTemp, curr.operand1);
        } else if (ins instanceof ArrayLoad) {
            ArrayLoad curr = (ArrayLoad) ins;
            addIfTemp(defTemp, curr.operand1);
            addIfTemp(useTemp, curr.arrayname);
        } else if (ins instanceof PrintCall) {
        	PrintCall curr = (PrintCall) ins;
        	if (curr.param1 != null) {
        		addIfTemp(useTemp, curr.param1);
        	}
        	
        } else if (ins instanceof ReadCall) { 
        	ReadCall curr = (ReadCall) ins;
        	addIfTemp(defTemp, curr.operand1);
    }	else if (ins instanceof ArrayAssign) {
        	ArrayAssign curr = (ArrayAssign) ins;
        	addIfTemp(defTemp, curr.arrayname);
        }
    }
