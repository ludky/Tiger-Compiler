...
@members {
   ...
   // private 
    ...
    public String generate(String s) {
        return irg.generate(s);
    }

    public String Igenerate(String s) {
        return irg.Igenerate(s);
    }
...
funct_declaration_tail
    :	FUNCTION Identifier{emit(irg.emitLabel($Identifier.text));}
        LPAREN param_list
                {
                    for(String param: $param_list.paramList) {
                       // String type = st.lookup(param).getTypeName();
                        emit(irg.emit_assign_stat(param, "0")); //change later ** initialize the params to 0 or 0.0 depends on the base type and emitted each params
                    }
                }
        ...
        LPAREN param_list
                {
                    for(String param: $param_list.paramList) {
                       // String type = st.lookup(param).getTypeName();
                        emit(irg.emit_assign_stat(param, "0")); //initialize the params to 0 or 0.0 depends on the base type and emitted each params
                    }
                }
        RPAREN BEGIN block_list END SEMI
	;
...
main_function
    :   MAIN {emit(irg.emitLabel($MAIN.text));} LPAREN RPAREN BEGIN block_list END SEMI
    ;
...
if_else_expr
    :	if_stat then_stat {emit(irg.emit_label($if_stat.label));} else_expr ENDIF SEMI
    ;
    
if_stat returns [String label]
	: IF expr {label = $expr.label;} //label of return addr
	;
...
expr returns [String val, String label]
    ...
    :   exp1 = expr_lev3 {
            String temp1 = $exp1.val;
            emit(irg.emit_assign_stat(temp1, $exp1.val));
        }
        ((AND {op = $AND.text;} | OR{$op = $OR.text;})
           exp2 =  expr_lev3
            {
                if (op.equals($AND.text)) {
                    emit(emit_brneq( , , label)); //**
                } else {
                    emit(emit_breq( , , label)); //**
                }
                String temp2 = $exp2.val;
                emit(irg.emit_assign_stat(temp1, $exp2.val));
            }
        )*
      //  emit(irg.emit_label(label));
        $val = r;
    ;

expr_lev3 returns [String val] // val is either value or label of branch
    {
        String op;
    }
    :   exp1 = expr_lev2 {$val = exp1.val;}
        ((EQ {op = $EQ.text;}|NEQ{op = $NEQ.text;}|LESSER{op = $LESSER.text;}|LESSEREQ{op = $LESSEREQ.text;}|GREATER{op = $GREATER.text;}|GREATEREQ{op = $GREATEREQ.text;})
            ...
            {
                String label = getNewLabel();
                emit(irg.emit_branch(op, $val, exp1.val, label));
                //**
            }
...
expr_lev2 returns [String val]
    ...
    :   exp1 = expr_lev1 {$val = exp1.val}
        ((PLUS {op = $PLUS.text;}|MINUS {op = $MINUS.text;})
            exp2 = expr_lev1
            {
                String r = getNewTemp();
                emit(irg.emit_binary_op(op, $val, exp2.val, r));
                $val = r;
            }
        )*
    ;
...
expr_lev1 returns [String val]
    ...
    :   val1 = primary_expression {$val = val1.val;}
        ((MULT {op = $MULT.text;}|DIV {op = $DIT.text;})
            val2 = primary_expression {
                String r = getNewTemp();
                if (op.equals($MULT.text)) {
                    emit(irg.emit_mult($val, val2.val, r));
                } else {
                    emit(irg.emit_div($val, val2.val, r));
                }
                $val = r; 
                })*
    ;
    ...
    :   Identifier {$ val = $Identifier.text;} value_tail
        {
            ...
            if(!$value_tail.offset.equals("null")) {
                emit(irg.emit_array_load(r, $val, Integer.parseInt($value_tail.offset)));
                $val = r;
            }
        }
    ;
...
value_tail returns [String offset]
    ...
    :   (LBRACK index_expr{$offset = index_expr.val} RBRACK
            ...
            String size = st.;
            emit(emit_mult($offset, size, r));//** get array size
            emit(emit_add(r, $value_tail_tail.idx, r));
        })?
    ;
